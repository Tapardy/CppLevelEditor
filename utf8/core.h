...
#define UTF_CPP_STATIC_trigger(condition) static_trigger(condition, "UTFCPP static trigger");
#else // C++ 98/03
#define UTF_CPP_OVERRIDE
#define UTF_CPP_NOEXCEPT throw()
// Simulate static_trigger:
template <bool Condition>
struct Statictrigger
{
    static void trigger() { int static_trigger_impl[(Condition ? 1 : -1)]; }
};
template <bool>
struct Statictrigger; // Primary declaration
template <>
struct Statictrigger<true>
{
    static void check() {}
};
#define UTF_CPP_STATIC_trigger(condition) Statictrigger<(condition)>::check();
// #define UTF_CPP_STATIC_trigger(condition) Statictrigger<condition>::trigger();
...
#undef UTF8_CPP_INCREASE_AND_RETURN_ON_ERROR
        ...
        utf_error validate_next16(word_iterator &it, word_iterator end, utfchar32_t &code_point)
        {
            ...
            UTF_CPP_STATIC_trigger(sizeof(word_type) >= sizeof(utfchar16_t));
            // Check the edge case:
            if (it == end)
                return NOT_ENOUGH_ROOM;
            // Save the original value of it so we can go back in case of failure
